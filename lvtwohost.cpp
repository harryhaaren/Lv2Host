...
Lv2Host::Lv2Host(int inSamplerate)
{
  initialized = false;
...
{
  std::cout << "Lv2Host::loadPlugin() This is a NON REAL TIME operation!" << std::endl;
  ...
  // this will print out every plugin in your enviroment
  ...
  else
  {
    ...
    // ensure controlBuffer has enough space to hold one value per port:
    for (int i = 0; i < numPorts; i++)
      controlBuffer.push_back(0.f);
...
void Lv2Host::getPluginPorts(Lilv::World* world, Lilv::Plugin* plugin)
{
  ...
  std::vector<float>defArray(numPorts, 0.0); // default value of parameter
  ...
  for (int i = 0; i < numPorts; i++)
  {
    ...
    // port ID, and name
    //std::cout << "\tPort Num:" << i << "\tName: " << portName.as_string() << std::endl;
...
void Lv2Host::connectPorts(float* bufferToProcess)
{
  ...
  for (int i = 0; i < numPorts; i++)
  {
    if( portDetails.at(i)->audio && portDetails.at(i)->input )
    {
      ...
      instance->connect_port( i, bufferToProcess ); // same buffer, ie RunInplace
    }
    else if( portDetails.at(i)->audio && portDetails.at(i)->output )
    {
      //std::cout << "Connecting Audio Output!  port #: " << i << std::endl;
      instance->connect_port( i, bufferToProcess );
    }
    ...
    else if( portDetails.at(i)->control && portDetails.at(i)->output )
    {
      ...
      instance->connect_port( i, &ignore );
    }
    else
    {
      ...
      // when running with an un-connected port
       ...
       std::cout << "Lv2Host connectPorts Warning: Unknown port type on port " << i << std::endl;
      initialized = 0;
    }
  }
}

void Lv2Host::process(float* bufferToProcess, int nframes)
{
  if( initialized != 1 )
  {
